...
class portable_analyzer():
    ...
    def Radiant_efficiency(self, DUT_cleardata, SLB_cleardata, FOVx = 16, FOVy = 8, pixperdeg = 18.75, exposure_SLB = 1, exposure_DUT = 1,dut_center_m = None, dut_center_n = None,slb_center_m = None,slb_center_n = None, ND_A_path = None, ND_B_path = None):                            
        [H,W] = self.clear.shape
        ...
        FOVx_2nd = 16
        FOVy_2nd = 8
        DUT_Y_crop_2nd = [round(dut_center_n - FOVy_2nd*pixperdeg/2), round(dut_center_n + FOVy_2nd*pixperdeg/2)]
        DUT_X_crop_2nd = [round(dut_center_m - FOVx_2nd*pixperdeg/2), round(dut_center_m + FOVx_2nd*pixperdeg/2)]
        SLB_Y_crop_2nd = [round(slb_center_n - FOVy_2nd*pixperdeg/2), round(slb_center_n + FOVy_2nd*pixperdeg/2)]
        SLB_X_crop_2nd = [round(slb_center_m - FOVx_2nd*pixperdeg/2), round(slb_center_m + FOVx_2nd*pixperdeg/2)]
        ...
        clear_crop_blur_2nd = median_filter(np.array(DUT_cleardata[DUT_Y_crop_2nd[0]:DUT_Y_crop_2nd[1],DUT_X_crop_2nd[0]:DUT_X_crop_2nd[1]]),size=filter_size)
        [clear_min_2nd, clear_max_2nd] = np.percentile(clear_crop_blur_2nd,[5,95])
        
        clear_mean = np.mean(clear_crop_blur)
        
        self.uniformity_dut = clear_max/clear_min
        self.uniformity_slb = slb_max / slb_min
        self.uniformity_dut_2nd = clear_max_2nd/clear_min_2nd
        
        self.slbclear_pattern = self.slbclear.copy()
        self.clear_pattern = self.clear.copy()
        
       # self.uniformity_dut =  (clear_max - clear_min)/clear_mean #luminance_non_unifomrity_DUT
        ...
        cv2.rectangle(self.clear_pattern, (DUT_X_crop_2nd[0],DUT_Y_crop_2nd[0]),(DUT_X_crop_2nd[1],DUT_Y_crop_2nd[1]), (30000, 30000, 30000), 2)
        
        return brightness_DUT, brightness_SLB, self.uniformity_dut, self.uniformity_slb
    ...
    def meancontrast(self,checker_m,checker_n,squaresize_m = 127, squaresize_n = 127,ROI_portion = 0.5,center_m = None,center_n = None):    #checker_m is number of columns, #checker_n is number of rows
        if center_m == None:
    ...
    def contrastcalibration(self):
        ...
        self.calibrated_mincontrast = np.min(self.calibrated_contrastcsv)
    ...
    #def Find_FOV(self, DUT_cleardata, SLB_cleardata, FOVx = 34, FOVy = 34, pixperdeg = 18.75, inletangle = 1, exposure_DUT = 1,dut_center_m = None, dut_center_n = None,slb_center_m = None,slb_center_n = None):
...
def image_enhancement_with_bg(img_src, img_bg):
    imgdiv = cv2.divide(img_src, img_bg, scale=255)
    return imgdiv
...
def get_checkerboard_center(src_img, cb_full_size, cb_detectable_size ,detectable_offset, degree=3):
   ...
   #cv2.imwrite('output_image_chessboard.png', imgcolor)
    ...
    #imgcolor = cv2.cvtColor(imgsrc,cv2.COLOR_GRAY2RGB)
    ## Radius of circle 
    #radius = 5
   ...
   # cv2.imwrite('output_image_center.png', imgcolor)
